...
project that schedules the Flexible Job-Shop Scheduling Problem (FJSP) to jointly minimise
...
creates both an opportunity and an incentive for energy-flexible manufacturers to practice \emph{demand
...
Minimising energy cost in isolation is an ill-posed objective: a scheduler that always waits for the
cheapest possible price window will produce arbitrarily long schedules. The problem is therefore
inherently \emph{bi-objective}, simultaneously minimising makespan (total completion time) and energy
cost, and the output of interest is not a single ``best'' schedule but a \emph{Pareto front} of
trade-offs between the two, from which a decision-maker can select according to their own priorities.

This project implements a memetic NSGA-II that approximates the makespan/energy-cost Pareto front for
...
\begin{itemize}
    ...
    solver to its memory limits; they are out of scope for a project of this scale run on commodity
    hardware.
...
produce makespan/energy-cost trade-offs grounded in real problem data rather than synthetic instances
or assumed energy rates. The approach and its motivation follow \citet{burmeister2024memetic}; this
document explicitly distinguishes what is reproduced from what is simplified relative to that source.
